...
public class DatabaseBackendQueueProcessor implements BackendQueueProcessor {
	...
	private void validate(Properties props) {
		if ( props.getProperty( HIBERATE_CONNECTION_DATASOURCE ) == null ) {
			throw log.configurationPropertyCantBeEmpty( HIBERATE_CONNECTION_DATASOURCE );
		}
	}
	...
	private void persistLuceneWorkListToDatabase(final List<LuceneWork> workList) {
		...
		if ( TransactionManagerHolder.getTransactionManager() == null ) {
			try {
				...
				jtaPlatformInstance.injectServices(
						( (EntityManagerFactoryImpl) EntityManagerFactoryHolder.getEntityManagerFactory() ).getSessionFactory().getServiceRegistry() );
